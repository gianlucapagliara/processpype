...
**Builder stage** --- installs all dependencies and the application into a virtual environment using `uv`.
...
CMD ["python", "-m", "uvicorn", "processpype.main:app.api", \
...
Configure the application via environment variables when running in Docker:
...
| `ENABLED_SERVICES` | `""` | Comma-separated list of services to auto-start |
